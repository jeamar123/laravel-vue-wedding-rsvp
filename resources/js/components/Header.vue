<template>
  <header
    ...
    :class="[
      ' ',
      isOtherPage || headerCoordinatesY >= browserHeight || isMobileMenuShown
        ? 'bg-[rgba(234,231,217,0.8)] md:bg-[#eae7d9] border-b border-[rgba(0,0,0,0.05)]'
        : 'bg-[rgba(0,0,0,0.10)] '
    ]"
  >
    ...
    <button
      ...
      :class="[
        ...
        isOtherPage || headerCoordinatesY >= browserHeight || isMobileMenuShown
          ? 'border-[#9f5b4e] bg-[#9f5b4e]'
          : 'border-white '
      ]"
      @click="isMobileMenuShown = !isMobileMenuShown"
    >
      <Bars3Icon class="w-10" />
    </button>
    ...
    >
      <template v-for="(el, index) in routes" :key="el">
        <div class="flex items-center gap-8">
          <router-link
            v-if="isOtherPage"
            as="a"
            :to="el === 'rsvp' ? { name: 'Rsvp' } : { name: 'Landingpage', hash: `#${el}` }"
            :class="[
          ...
          <a
            v-else
            :href="`#${el}`"
            class="px-5 md:px-0 py-2 md:py-3 text-xl md:text-3xl font-bold capitalize cursor-pointer tracking-[3px] c-transition w-full md:w-auto"
            :class="[
              headerCoordinatesY >= browserHeight
                ? compareCoords(`#${el}`)
                  ? 'text-[#952d1b] underline'
